...
// All saved-opportunity routes require authentication.
  ...
  try {
    const result =
      await pool.query(
        ...
        ORDER BY
          saved_opportunities.saved_at DESC
        `,
        [req.user.id]
      );
...
router.post("/", async (req, res) => {
  try {
    ...
    if (
      opportunityResult.rows.length === 0
    ) {
    ...
    // Check whether this opportunity
    // is already saved by this user
